...
type StorageEngine struct {
	...
	SupportsCommittedREads bool   `bson:supportsCommittedReads"` // 原来是 SupportCommittedREads，这样获取不到结果，percona写错了
...
type MemStats struct {
	...
	Note              interface{} `bson:"note"` // 如果mem.supported为假，则该字段出现 。该mem.note字段包含文本：。"not all mem info support on this platform"
}
