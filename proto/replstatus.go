...
type Members struct {
	Optime map[string]Optime `bson:"optimes"` // See Optime struct     ？？？？？？what bugs 这tm写错了 拼错了啊，optime -- > optimes ？？？？
	//Optime      map[string]Optime  `bson:"optime"`      // See Optime struct
	...
	//
	//
	//Uptime               string              `bson:"uptime"`               // number of seconds that this member has been online.
...
type ReplicaSetStatus struct {
	...
	ElectionCandidateMetrics ElectionCandidateMetrics `bson:"electionCandidateMetrics"`
}

type Member struct {
	// db.runCommand({replSetGetStatus:1}).members
...
type ElectionCandidateMetrics struct {
	// db.runCommand({replSetGetStatus:1}).electionCandidateMetrics
	LastElectionReason string             `bson:"lastElectionReason"`
	LastElectionDate   primitive.DateTime `bson:"lastElectionDate"` // decode ISODate
}
